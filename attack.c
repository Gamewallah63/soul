...
int main(int argc, char *argv[]) {
    // Expiration date: October 25, 2024, 11:59:00
    struct tm expiration_tm = {0};
    expiration_tm.tm_year = 2024 - 1900; // Year since 1900
    expiration_tm.tm_mon = 10 - 1;        // Months since January (0-11)
    expiration_tm.tm_mday = 25;           // Day of the month
    expiration_tm.tm_hour = 11;           // Hour (0-23)
    expiration_tm.tm_min = 59;            // Minute (0-59)
    expiration_tm.tm_sec = 0;              // Second (0-60)
    time_t expiration_time = mktime(&expiration_tm);

    // Check expiration
    if (time(NULL) > expiration_time) {
        printf("This binary has expired.\n");
        exit(1);
    }
    ...
    for (int i = 0; i < threads; i++) {
        ...
        printf("Launched thread with ID: @jks6190 %lu\n", thread_ids[i]);
    }
    ...
    free(thread_ids);
    printf("Attack finished join @jks6190\n");
    return 0;
}
